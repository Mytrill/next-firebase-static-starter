...
import {
    DocumentHookQuery, FetchedDocument, PathOrQuery, PathsOrQueries, Query, QueryCondition,
    QueryOrderBy
} from "./types"
...
export async function fetchDocumentByPathOrQuery<T>(
  pathOrQuery: PathOrQuery
): Promise<(FetchedDocument & T) | undefined> {
  ...
  if (!conditions) {
    return fetchDocument<T>(path)
  }

  const docs = await fetchCollection<T>(path, { conditions, limit: 1 })
