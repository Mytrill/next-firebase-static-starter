...
import { asyncEffect } from "@common/utils/asyncEffect"

import { setDocument } from "./setDocument"
import { PathOrQuery } from "./types"
import { updateDocument } from "./updateDocument"
import { fetchDocumentByPathOrQuery, getPathOrQueryDeps } from "./utils"
...
export function useDocument<T>(
  ...
  useEffect(
    asyncEffect(async () => {
      ...
      setLoading(true)
      try {
        const values = await fetchDocumentByPathOrQuery<T>(pathOrQuery)
        if (!values) {
    ...
    set: async (values: T) => {
      ...
      if (options.set) {
        await options.set({ path, values, exists }, values => {
          actualValues = values
          return setDocument(path, values)
        })
      } else {
        await setDocument(path, values)
      }
    ...
    update: async (update: Partial<T>) => {
      ...
      if (options.update) {
        await options.update({ existing: values, path, update }, values => {
          actualUpdate = values
          return updateDocument(path, values)
        })
      } else {
        await updateDocument(path, update)
      }
